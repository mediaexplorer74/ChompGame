...
namespace GameManager
{
	class PauseDialog : Sprite
	{
		...
		public PauseDialog(Game game)
		{
			...
			{
				new OnOffMenuItem 
				{ Off = game.Content.Load<Texture2D>("gfx\\dialog\\ResumeBtnOff"),
					On = game.Content.Load<Texture2D>("gfx\\dialog\\ResumeBtnOn") },
				new OnOffMenuItem 
				{ Off = game.Content.Load<Texture2D>("gfx\\dialog\\ExitBtnOff"),   
					On = game.Content.Load<Texture2D>("gfx\\dialog\\ExitBtnOn") },
			};
				...
				if(InputManager.CurrentState.Start)
				{
					switch(_selectedIndex)
					{
						...
						case 1:
							...
							Game1.Instance.SetState(GameState.TitleScreen);
							break;
					}

					AudioManager.SoundEffects[AudioManager.Cue.MenuSelect].Play();
					Game1.Instance.IsPaused = !Game1.Instance.IsPaused;
				}
