...
namespace GameManager
{
	...
	public static class FontManager
	{
		...
		private static Dictionary<FontSize,SpriteFont> _fontMap
			= new Dictionary<FontSize,SpriteFont>()
			{
				{ FontSize.Regular, Game1.Instance.Content.Load<SpriteFont>("Font") },
				{ FontSize.Large, Game1.Instance.Content.Load<SpriteFont>("LargeFont") },
			};
		...
		public static void DrawLine(SpriteBatch spriteBatch, FontSize size, string text, Vector2 pos, float scale, float rotation, Color color, Alignment align)
		{
			...
			switch(align)
			{
				...
				case Alignment.Right:
					alignPos = new Vector2(Game1.ScreenWidth - stringSize.X, YPosition);
					break;
			}

			// draw the string and increment the vertical position
			spriteBatch.DrawString(_fontMap[size], text, pos, color, rotation, alignPos, 
