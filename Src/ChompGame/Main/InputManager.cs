...
namespace GameManager
{
	...
	public static class InputManager
	{
		...
		private static readonly Dictionary<InputStates, Keys> KeyMap 
			= new Dictionary<InputStates, Keys>();
		...
		public static void Update(GameTime gameTime)
		{
			...
			// for each button, get it's actual state (we only want one "true" per button press...
			// holding a button doesn't stay "true"
			_currentInputState.Up = GetKeyState(_gamePadState.DPad.Up,
				_lastGamePadState.DPad.Up, InputStates.Up);
			_currentInputState.Down = GetKeyState(_gamePadState.DPad.Down,
				_lastGamePadState.DPad.Down, InputStates.Down);
			_currentInputState.Left = GetKeyState(_gamePadState.DPad.Left,
				_lastGamePadState.DPad.Left, InputStates.Left);
			_currentInputState.Right = GetKeyState(_gamePadState.DPad.Right,
				_lastGamePadState.DPad.Right, InputStates.Right);

			_currentInputState.Back = GetKeyState(_gamePadState.Buttons.Back,
				_lastGamePadState.Buttons.Back, InputStates.Back) |
										GetKeyState(_gamePadState.Buttons.B, 
										_lastGamePadState.Buttons.B, InputStates.Back) |
										GetKeyState(_mouseState.RightButton, 
										_lastMouseState.RightButton, InputStates.Back);
			_currentInputState.Start = GetKeyState(_gamePadState.Buttons.Start,
				_lastGamePadState.Buttons.Start, InputStates.Start) | 
										GetKeyState(_gamePadState.Buttons.A, 
										_lastGamePadState.Buttons.A, InputStates.Start) |
										GetKeyState(_mouseState.LeftButton, 
										_lastMouseState.LeftButton, InputStates.Start);

			_currentInputState.Debug = GetKeyState(_gamePadState.Buttons.LeftShoulder,
				_lastGamePadState.Buttons.LeftShoulder, InputStates.Debug);
			_currentInputState.FullScreen = GetKeyState(_gamePadState.Buttons.RightShoulder,
				_lastGamePadState.Buttons.RightShoulder, InputStates.FullScreen);

			// now setup a state for the real gamepad state ("true" for the length of time held down)
			_rawInputState.Up = GetKeyState(_gamePadState.DPad.Up, InputStates.Up)
				| GetThumbstickState(InputStates.Up);
			_rawInputState.Down = GetKeyState(_gamePadState.DPad.Down, InputStates.Down) 
				| GetThumbstickState(InputStates.Down);
			_rawInputState.Left = GetKeyState(_gamePadState.DPad.Left, InputStates.Left) 
				| GetThumbstickState(InputStates.Left);
			_rawInputState.Right = GetKeyState(_gamePadState.DPad.Right, InputStates.Right)
				| GetThumbstickState(InputStates.Right);
		...
		private static bool GetKeyState(ButtonState buttonState, ButtonState lastButtonState, 
			InputStates map)
		{
			// single press
		...
		public static void SetVibration(float left, float right, 
			double durationLeft, double durationRight)
		{
