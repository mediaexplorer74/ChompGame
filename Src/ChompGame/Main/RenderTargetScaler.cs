	...
	class RenderTargetScaler
	{
		...
		public void Draw()
		{
			...
			PresentationParameters presentation = 
				_graphicsDeviceManager.GraphicsDevice.PresentationParameters;

			float outputAspect = 
				_game.Window.ClientBounds.Width / (float)_game.Window.ClientBounds.Height;
		...
		public Vector2 ScaleInput(Vector2 input)
		{
			...
			int presentHeight = (int)((_game.Window.ClientBounds.Width / preferredAspect) + 0.5f);
			int barHeight = (int)MathHelper.Clamp((_game.Window.ClientBounds.Height - presentHeight) 
				/ 2.0f, 0, _screenHeight);

			int presentWidth = (int)((_game.Window.ClientBounds.Height * preferredAspect) + 0.5f);
			int barWidth = (int)MathHelper.Clamp((_game.Window.ClientBounds.Width - presentWidth)
				/ 2.0f, 0, _screenWidth);

			Vector2 scale = new Vector2((float)(_game.Window.ClientBounds.Width - (barWidth * 2)) 
				/ _screenWidth,
										(float)(_game.Window.ClientBounds.Height - (barHeight * 2))
										/ _screenHeight);
