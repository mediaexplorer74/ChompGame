	...
	public class TitleScreen : Screen
	{
        ...
        public TitleScreen(Game game) : base(game)
		{
			...
			_menuItems = new List<OnOffMenuItem>()
			{
				new OnOffMenuItem 
				{ 
					Off = game.Content.Load<Texture2D>("gfx\\title\\StartBtnOff"),        
					On = game.Content.Load<Texture2D>("gfx\\title\\StartBtnOn")
				},
				//new OnOffMenuItem { Off = game.Content.Load<Texture2D>("gfx\\title\\OptionsBtnOff"),
				//On = game.Content.Load<Texture2D>("gfx\\title\\OptionsBtnOn") },
				new OnOffMenuItem 
				{ 
					Off = game.Content.Load<Texture2D>("gfx\\title\\AchievementsBtnOff"),
					On = game.Content.Load<Texture2D>("gfx\\title\\AchievementsBtnOn")
				},
				new OnOffMenuItem
				{ 
					Off = game.Content.Load<Texture2D>("gfx\\title\\LeaderboardBtnOff"), 
					On = game.Content.Load<Texture2D>("gfx\\title\\LeaderboardBtnOn")
				},
			};
			...
			{
				Debug.WriteLine("[ex]TitleScreen - AudioManager.SoundEffectInstances.Play bug: " + ex.Message);
			}

			for(int i = 0; i < _menuItems.Count; i++)
			{
				Vector2 position = MenuPosition + new Vector2(0, (i*_menuItems[i].On.Height));
				_menuItems[i].Position = 
					new Rectangle
					(
						(int)position.X, 
						(int)position.Y, 
						_menuItems[i].On.Width, 
						_menuItems[i].On.Height 
					);
			}
        }//TitleScreen
        ...
        public override void Update(GameTime gameTime)
		{
			for(int i = 0; i < _menuItems.Count; i++)
			{
				if(_menuItems[i].Position.Contains(Game1.Instance.Scaler.ScaleInput(InputManager.CurrentState.Position.ToVector2()).ToPoint()))
					_selectedIndex = i;
			}
			...
			switch(_state)
			{
					...
					if(InputManager.CurrentState.Start)
					{
						switch(_selectedIndex)
						{
							case 0:
								...
								break;
							case 1:
								Game1.Instance.SetState(GameState.Achievements);
								break;
							case 2:
								Game1.Instance.SetState(GameState.Leaderboard);
								break;
							default:
								//RnD
				...
				case TitleState.Start:
					if (1==1)//(AudioManager.SoundEffectInstances[AudioManager.Cue.Start].State != SoundState.Playing)
					{
						Game1.Instance.SetState(GameState.GameScreen);
					}
					break;
				default:
			...
			for (int i = 0; i < _menuItems.Count; i++)
			{
				...
				(
					(_selectedIndex == i) 
					  ? _menuItems[i].On 
					  : _menuItems[i].Off, 
					MenuPosition + new Vector2(0, (i * _menuItems[i].On.Height)), 
					Color.White 
				);
			}
